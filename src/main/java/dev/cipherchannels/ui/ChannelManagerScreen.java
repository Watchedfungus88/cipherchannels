...
public final class ChannelManagerScreen extends Screen {
    ...
    @Override
    protected void init() {
        ChannelConfig config = CipherChannelsClient.channels().config();
    ...
    private void initHeader(int left, int panelWidth, ChannelConfig config) {
        ChannelStatus status = CipherChannelsClient.channels().status(ClientContext.currentServer());
        ...
        addRenderableOnly(new StringWidget(left, 42, panelWidth, 12,
            Component.translatable("cipherchannels.header.endpoint_transport", endpoint,
                ClientContext.transportName(CipherChannelsClient.channels().transportFor(ClientContext.currentServer())))
    ...
    private void initOverview(int left, int panelWidth, ChannelConfig config) {
        ...
        addText(left, y, panelWidth, 1, Component.translatable("cipherchannels.overview.key",
            active != null && CipherChannelsClient.channels().hasSessionKey(active.id())
        ...
        y += 28;
        TransportMode mode = CipherChannelsClient.channels().transportFor(ClientContext.currentServer());
        ...
        if (!enabled && (active == null || !CipherChannelsClient.channels().canKeepEncryptionOn(active.id(), ClientContext.currentServer()))) {
            disable(toggle, Component.translatable(active == null
                ? "cipherchannels.disabled.no_channel" : "cipherchannels.disabled.not_ready"));
        }
        y += 34;

        ChannelRecord bound = CipherChannelsClient.channels().boundChannelFor(ClientContext.currentServer());
    ...
    private void initChannelForm(int left, int panelWidth, int y, boolean onboarding) {
        ...
        ChannelRecord active = CipherChannelsClient.channels().config().activeChannel();
    ...
    private void initShare(int left, int panelWidth, ChannelConfig config) {
        ...
        y += 30;
        int half = (panelWidth - 8) / 2;
        TransportMode mode = CipherChannelsClient.channels().transportFor(endpoint);
    ...
    private void submitForm() {
        runAction(() -> {
            switch (form) {
                case CREATE -> {
                    ChannelRecord created = CipherChannelsClient.channels().create(nameInput.getValue());
                ...
                case JOIN -> {
                    ChannelRecord joined = CipherChannelsClient.channels().importInvite(inviteInput.getValue(), nameInput.getValue());
                ...
                case RENAME -> {
                    ...
                    CipherChannelsClient.channels().rename(active.id(), nameInput.getValue());
                    feedback = new UiFeedback(UiFeedback.Severity.SUCCESS,
    ...
    private void toggleEncryption() {
        runAction(() -> {
            boolean enabled = CipherChannelsClient.channels().config().encryptionEnabled();
            CipherChannelsClient.channels().setEnabled(!enabled, ClientContext.currentServer());
    ...
    private void requestSwitch(ChannelRecord target) {
        ...
        if (flow == UiPolicy.SwitchFlow.CONFIRM) {
            minecraft.gui.setScreen(new ChannelSwitchScreen(this, target));
            return;
        }
    ...
    void performSwitch(ChannelRecord target, boolean keepEncryptionOn) {
        runAction(() -> {
            CipherChannelsClient.channels().select(target.id(), keepEncryptionOn, ClientContext.currentServer());
    ...
    private void confirmCopyInvite(ChannelRecord active) {
        BooleanConsumer callback = accepted -> {
            if (accepted) {
                runAction(() -> {
                    minecraft.keyboardHandler.setClipboard(CipherChannelsClient.channels().inviteFor(active.id()));
        ...
        minecraft.gui.setScreen(new ConfirmScreen(callback,
    ...
    private void confirmForget() {
        ...
        minecraft.gui.setScreen(new ConfirmScreen(accepted -> {
            if (accepted) {
                runAction(() -> {
                    CipherChannelsClient.channels().forget(active.id());
    ...
    private void setTransport(TransportMode mode) {
        runAction(() -> {
            CipherChannelsClient.channels().setTransport(ClientContext.currentServer(), mode);
