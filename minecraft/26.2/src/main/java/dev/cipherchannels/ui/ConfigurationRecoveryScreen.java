...
public final class ConfigurationRecoveryScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.translatable(recovered
            ? "cipherchannels.recovery.continue" : "cipherchannels.recovery.reset"), ignored -> {
                if (recovered) { CipherChannels.channels().acknowledgeRecovery(); minecraft.gui.setScreen(new ChannelManagerScreen(parent)); }
                else confirmReset();
    ...
    private void confirmReset() {
        BooleanConsumer callback = accepted -> {
            if (accepted) { CipherChannels.channels().resetUnsafeConfiguration(); minecraft.gui.setScreen(new ChannelManagerScreen(parent)); }
            else minecraft.gui.setScreen(this);
        };
        minecraft.gui.setScreen(new ConfirmScreen(callback, Component.translatable("cipherchannels.recovery.confirm.title"),
