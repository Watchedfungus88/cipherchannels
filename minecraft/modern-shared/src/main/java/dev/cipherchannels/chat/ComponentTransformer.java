...
public final class ComponentTransformer {
    ...
    public Component transform(Component source) {
        ...
        if (flattened != null) return TransformedMessageRegistry.mark(flattened.component);
    ...
    private TransformResult transformAcrossNodes(Component source) {
        ...
        for (int start = 0; start < parts.size();) {
            if (parts.get(start).component != null) { start++; continue; }
            int end = start;
            ...
            for (FrameCandidate candidate : FrameScanner.scan(text.toString())) {
                if (leafAt(parts, start, end, candidate.start()) != leafAt(parts, start, end, candidate.endExclusive() - 1)) { crossing = true; break; }
            }
            if (crossing) break;
            start = end;
        }
        ...
        for (int start = 0; start < parts.size();) {
            ...
            if (part.component != null) { result.append(transformInternal(part.component).component); start++; continue; }
            int end = start;
    ...
    private void flatten(Component source, Style inherited, List<FlatPart> parts) {
        ...
        if (source.getContents() instanceof PlainTextContents plain) parts.add(new FlatPart(plain.text(), effective, null));
        else parts.add(new FlatPart(null, null, source.plainCopy().withStyle(effective)));
    ...
    private void appendRun(MutableComponent result, List<FlatPart> parts, int start, int end) {
        ...
        for (FrameCandidate candidate : FrameScanner.scan(text.toString())) {
            ...
            } else if (incoming.failure() == FrameFailure.REPLAYED) result.append(Component.translatable("cipherchannels.chat.replay_blocked").withStyle(ChatFormatting.RED));
            else { appendRange(result, parts, start, end, candidate.start(), candidate.endExclusive()); result.append(diagnosticBadge(incoming.failure())); }
            cursor = candidate.endExclusive();
        }
        appendRange(result, parts, start, end, cursor, text.length());
    }
    private static void appendRange(MutableComponent result, List<FlatPart> parts, int start, int end, int from, int to) {
        int offset = 0;
    ...
    private static int leafAt(List<FlatPart> parts, int start, int end, int position) {
        int offset = 0;
        for (int index = start; index < end; index++) { offset += parts.get(index).text.length(); if (position < offset) return index; }
        return end - 1;
    }
    ...
    private static Component authenticatedBadge(String name) {
        ...
        return Component.translatable("cipherchannels.chat.badge.label").withStyle(style ->
            style.withColor(ChatFormatting.AQUA).withHoverEvent(new HoverEvent.ShowText(tooltip)));
    }
    ...
    private static Style plaintextStyle(Style original, String wire) {
        ...
        if (original.getHoverEvent() instanceof HoverEvent.ShowText existing) {
            tooltip.append("\n\n").append(Component.translatable("cipherchannels.chat.hover.original"))
                .append("\n").append(existing.value());
        }
        return original.withHoverEvent(new HoverEvent.ShowText(tooltip));
    }
