...
public final class ConfigurationRecoveryScreen extends Screen {
    ...
    public ConfigurationRecoveryScreen(Screen parent) {
        super(Component.translatable("cipherchannels.recovery.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addRenderableOnly(new MultiLineTextWidget(left, 56,
            Component.translatable("cipherchannels.recovery." + CipherChannels.channels().loadState().name().toLowerCase()), font)
            .setMaxWidth(panelWidth).setMaxRows(6));
        ...
        addRenderableWidget(Button.builder(Component.translatable(recovered
            ? "cipherchannels.recovery.continue" : "cipherchannels.recovery.reset"), ignored -> {
                if (recovered) {
                    CipherChannels.channels().acknowledgeRecovery();
                    minecraft.setScreen(new ChannelManagerScreen(parent));
                } else {
                    confirmReset();
                }
    ...
    private void confirmReset() {
        ...
        minecraft.setScreen(new ConfirmScreen(callback, Component.translatable("cipherchannels.recovery.confirm.title"),
            Component.translatable("cipherchannels.recovery.confirm.message"),
            Component.translatable("cipherchannels.recovery.confirm.yes"), Component.translatable("gui.cancel")));
    }
