...
public final class ChannelManagerScreen extends Screen {
    enum Tab { OVERVIEW, CHANNELS, CHANNEL }
    ...
    protected void init() {
        ...
        if (CipherChannels.channels().loadState().locked()) {
            contentScroller.reset(82, height - 36);
            firstContentChild = children().size();
            contentLayout = true;
            initLocked(left, panelWidth);
        } else if (config.channels().isEmpty()) {
            ...
            switch (tab) {
                ...
                case CHANNEL -> initChannel(left, panelWidth, config);
            }
        }
        contentLayout = false;
    ...
    private void initLocked(int left, int panelWidth) {
        int y = 88;
        addText(left, y, panelWidth, 5, Component.translatable("cipherchannels.locked."
            + CipherChannels.channels().loadState().name().toLowerCase()), ChatFormatting.RED);
        y += 72;
        int half = (panelWidth - 8) / 2;
        addRenderableWidget(Button.builder(Component.translatable("cipherchannels.locked.open_folder"),
            ignored -> ClientContext.openConfigFolder()).bounds(left, y, half, 22).build());
        addRenderableWidget(Button.builder(Component.translatable("cipherchannels.locked.reset"),
            ignored -> confirmReset()).bounds(left + half + 8, y, half, 22).build());
    }
    ...
    private void initTabs(int left, int panelWidth) {
        ...
        addTabButton(left + (buttonWidth + gap) * 2, 68, buttonWidth, Tab.CHANNEL,
            "cipherchannels.tab.channel");
    }
    ...
    private void initChannel(int left, int panelWidth, ChannelConfig config) {
        ChannelRecord active = config.activeChannel();
        if (active == null) {
            addText(left, 134, panelWidth, 3, Component.translatable("cipherchannels.channel_tab.no_channel"), ChatFormatting.YELLOW);
            return;
        }
        int y = 128;
        addText(left, y, panelWidth, 1, Component.translatable("cipherchannels.channel_tab.active", active.name()), ChatFormatting.AQUA);
        y += 20;
        addText(left, y, panelWidth, 2, Component.translatable("cipherchannels.channel_tab.fingerprint",
            active.fingerprint()), ChatFormatting.WHITE);
        y += 30;
        addRenderableWidget(Button.builder(Component.translatable("cipherchannels.channel_tab.copy_fingerprint"),
            ignored -> copyFingerprint(active)).bounds(left, y, panelWidth, 22).build());
        y += 30;
        addText(left, y, panelWidth, 2, Component.translatable("cipherchannels.channel_tab.invite_warning"), ChatFormatting.YELLOW);
        y += 30;
        Button copyInvite = addRenderableWidget(Button.builder(Component.translatable("cipherchannels.channel_tab.copy_invite"),
            ignored -> copyInvite(active)).bounds(left, y, panelWidth, 22).build());
        if (!CipherChannels.channels().hasSessionKey(active.id())) {
            disable(copyInvite, Component.translatable("cipherchannels.disabled.not_ready"));
        }
        y += 32;
        ...
        Button binding = addRenderableWidget(Button.builder(Component.translatable(active.binding() == null
                ? "cipherchannels.channel_tab.bind" : "cipherchannels.channel_tab.unbind"), ignored -> toggleBinding(active))
        ...
        y += 32;
        addText(left, y, panelWidth, 2, Component.translatable("cipherchannels.channel_tab.transport_help"), ChatFormatting.GRAY);
        y += 30;
        int half = (panelWidth - 8) / 2;
        ...
        y += 32;
        addRenderableWidget(Button.builder(Component.translatable("cipherchannels.channel_tab.rotate"), ignored -> confirmRotate(active))
            .tooltip(Tooltip.create(Component.translatable("cipherchannels.channel_tab.rotate_help"))).bounds(left, y, panelWidth, 22).build());
    }
            ...
            switch (form) {
                case CREATE -> {
                    ...
                    feedback = new UiFeedback(UiFeedback.Severity.SUCCESS,
                        Component.translatable("cipherchannels.feedback.created", created.name()));
                    tab = Tab.CHANNEL;
                }
                case JOIN -> {
                    ...
                    inviteDraft = "";
                    feedback = new UiFeedback(UiFeedback.Severity.SUCCESS,
                        Component.translatable("cipherchannels.feedback.joined", joined.name()));
                    tab = Tab.OVERVIEW;
                }
                case RENAME -> {
    ...
    private void requestSwitch(ChannelRecord target) {
        ChannelConfig config = CipherChannels.channels().config();
        if (target.id().equals(config.activeChannelId())) {
            setFeedback(Component.translatable("cipherchannels.feedback.already_active", target.name()), ChatFormatting.AQUA);
            return;
        }
        if (config.encryptionEnabled()
            && !CipherChannels.channels().canKeepEncryptionOn(target.id(), ClientContext.currentServer())) {
            setFeedback(Component.translatable("cipherchannels.disabled.not_ready"), ChatFormatting.RED);
            return;
        }
        performSwitch(target);
    }

    private void performSwitch(ChannelRecord target) {
        runAction(() -> {
            boolean enabled = CipherChannels.channels().config().encryptionEnabled();
            CipherChannels.channels().select(target.id(), ClientContext.currentServer());
            setFeedback(Component.translatable(enabled
    ...
    private void confirmRotate(ChannelRecord active) {
        minecraft.setScreen(new ConfirmScreen(accepted -> {
            if (accepted) runAction(() -> { ChannelRecord replacement = CipherChannels.channels().rotateKey(active.id()); InviteClipboardGuard.clearMatching(active.id());
                setFeedback(Component.translatable("cipherchannels.feedback.rotated", replacement.name()), ChatFormatting.YELLOW); });
            minecraft.setScreen(this);
        }, Component.translatable("cipherchannels.confirm.rotate.title"), Component.translatable("cipherchannels.confirm.rotate.message", active.name()),
            Component.translatable("cipherchannels.confirm.rotate.yes"), Component.translatable("gui.cancel")));
    }

    private void copyFingerprint(ChannelRecord active) {
        minecraft.keyboardHandler.setClipboard(active.fingerprint());
        setFeedback(Component.translatable("cipherchannels.feedback.fingerprint_copied"), ChatFormatting.GREEN);
    }

    private void copyInvite(ChannelRecord active) {
        runAction(() -> {
    ...
    private void confirmReset() {
        minecraft.setScreen(new ConfirmScreen(accepted -> {
            if (accepted) runAction(() -> {
                ...
                setFeedback(Component.translatable("cipherchannels.feedback.config_reset"), ChatFormatting.YELLOW);
            });
            minecraft.setScreen(this);
